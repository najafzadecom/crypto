    ...
    <div class="content">
        <div class="card">
            <div class="card-header d-flex">
                ...
                <div class="ms-auto">
                    <x-buttons.create title="{{ __('Create') }}" url="{{ route('admin.pages.create') }}" permission="pages-create"/>
                </div>
            </div>
            ...
            <div class="table-responsive">
                <table class="table text-nowrap">
                    <thead>
                    <tr>
                        {!! sortableTableHeader('id', 'ID', 'pages') !!}
                        <th>{{ __('Title') }}</th>
                        <th>{{ __('Meta Title') }}</th>
                        {!! sortableTableHeader('status', 'Status', 'pages') !!}
                        {!! sortableTableHeader('created_at', 'Created At', 'pages') !!}
                    ...
                    @forelse($items as $item)
                        @php
                            ...
                            $metaTitle = $defaultTranslation?->meta_title ?? $item->translations->first()?->meta_title ?? '-';
                        @endphp
                        <tr>
                            <td>{{ $item->id }}</td>
                            <td>
                            ...
                            <td>
                                <div class="text-muted">{{ Str::limit($metaTitle, 40) }}</div>
                            </td>
                            <td>{!! $item->status_html !!}</td>
                            <td>{{ $item->created_at->format('d.m.Y H:i') }}</td>
                            <td class="text-center">
                                @canany(['pages-show', 'pages-edit', 'pages-delete'])
                                <div class="dropdown">
                                    ...
                                    <div class="dropdown-menu dropdown-menu-end">
                                        ...
                                        @can('pages-show')
                                        <a href="#" class="dropdown-item" data-url="{{ route('admin.pages.show', $item->id) }}" data-bs-toggle="modal" data-bs-target="#show_modal">
                                            <i class="ph-eye me-2"></i>
                                            {{ __('Show page') }}
                                        </a>
                                        @endcan
                                        @can('pages-edit')
                                        <a href="{{ route('admin.pages.edit', $item->id) }}" class="dropdown-item">
                                        ...
                                        <a href="#" class="dropdown-item text-danger"
                                           data-delete-url="{{ route('admin.pages.destroy', $item->id) }}"
