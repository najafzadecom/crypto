    ...
    <div class="content">
        <div class="card">
            <div class="card-header d-flex">
                ...
                <div class="ms-auto">
                    <x-buttons.create title="{{ __('Create') }}" url="{{ route('admin.packages.create') }}" permission="packages-create"/>
                </div>
            </div>
                        ...
                        <div class="col-lg-2">
                            <div class="mb-3">
                                ...
                                <select name="is_popular" class="form-select">
                                    ...
                                    <option value="1"{{ request('is_popular') == '1' ? ' selected' : '' }}>{{ __('Popular') }}</option>
                                    <option value="0"{{ request('is_popular') == '0' ? ' selected' : '' }}>{{ __('Not Popular') }}</option>
                                </select>
                            </div>
                        </div>
            ...
            <div class="table-responsive">
                <table class="table text-nowrap">
                    <thead>
                    <tr>
                        {!! sortableTableHeader('id', 'ID', 'packages') !!}
                        <th>{{ __('Name') }}</th>
                        <th>{{ __('Price') }}</th>
                        <th>{{ __('Duration') }}</th>
                        <th>{{ __('Popular') }}</th>
                        {!! sortableTableHeader('status', 'Status', 'packages') !!}
                        {!! sortableTableHeader('created_at', 'Created At', 'packages') !!}
                    ...
                    @forelse($items as $item)
                        ...
                        <tr>
                            ...
                            <td>
                                @if($item->is_popular)
                                    <span class="badge bg-warning">{{ __('Popular') }}</span>
                                @else
                            ...
                            <td class="text-center">
                                ...
                                <div class="dropdown">
                                    ...
                                    <div class="dropdown-menu dropdown-menu-end">
                                        ...
                                        @can('packages-show')
                                        <a href="#" class="dropdown-item" data-url="{{ route('admin.packages.show', $item->id) }}" data-bs-toggle="modal" data-bs-target="#show_modal">
                                            <i class="ph-eye me-2"></i>
                                            {{ __('Show package') }}
                                        </a>
                                        @endcan
                                        @can('packages-edit')
                                        <a href="{{ route('admin.packages.edit', $item->id) }}" class="dropdown-item">
                                        ...
                                        <a href="#" class="dropdown-item text-danger"
                                           data-delete-url="{{ route('admin.packages.destroy', $item->id) }}"
                                           data-item-name="{{ $name }}">
                        ...
                        <tr>
                            <td colspan="8" class="text-center py-4">
                                <div class="text-muted">
                                    <i class="ph-package display-6 d-block mb-2"></i>
